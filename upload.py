...
def main():
    ...
    project_home = f"/home/{username}/gemini-server"
    ...
    if resp.status_code != 200:
        print(
            f"Error getting website list: status was {resp.status_code}\n{resp.content}")
        sys.exit(-1)
    ...
    if site_hostname not in sites:
        print(
            f"Creating website at {site_hostname} with POST to {webapps_url}")
        resp = requests.post(
            webapps_url,
            data={
        ...
        if resp.status_code not in (200, 201):
            print(
                f"Error creating site: status was {resp.status_code}\n{resp.content}")
            sys.exit(-1)
    ...
    for path in paths_to_upload:
        ...
        if resp.status_code not in (200, 201):
            print(
                f"Error uploading {path}: status was {resp.status_code}\n{resp.content}")
            sys.exit(-1)
    ...
    if resp.status_code not in (200, 201):
        print(
            f"Error uploading WSGI file: status was {resp.status_code}\n{resp.content}")
        sys.exit(-1)

    our_webapp_url = urljoin(webapps_url, f"{site_hostname}/")
    static_file_route_url = urljoin(our_webapp_url, "static_files/")
    print(
        f"Getting existing static file routes with get to {static_file_route_url}")
    resp = requests.get(
    ...
    if resp.status_code != 200:
        print(
            f"Error getting static file route list: status was {resp.status_code}\n{resp.content}")
        sys.exit(-1)
    ...
    if "/static" not in static_route_urls:
        print(
            f"Configuring static file route with post to {static_file_route_url}")
        resp = requests.post(
            static_file_route_url,
            data={
                "url": "/static",
        ...
        if resp.status_code not in (200, 201):
            print(
                f"Error creating static file route: status was {resp.status_code}\n{resp.content}")
            sys.exit(-1)
    ...
    if resp.status_code not in (200, 201):
        print(
            f"Error reloading website: status was {resp.status_code}\n{resp.content}")
        sys.exit(-1)
